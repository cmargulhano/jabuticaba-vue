...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=jaboticaba.example.com
...
workon jaboticaba  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development jaboticaba npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop jaboticaba
    docker rm jaboticaba
    docker run --name jaboticaba -d --env-file /home/ubuntu/jaboticaba.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/jaboticaba:/dkdata \
        jaboticaba start_web.sh
}
...
echo_green "Welcome to jaboticaba's dev env"
